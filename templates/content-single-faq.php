...
<?php
	$client = get_post_meta( get_the_ID(), 'lsx_faq_client', true );
	$client_logo = get_post_meta( get_the_ID(), 'lsx_faq_client_logo', true );
	$url = get_post_meta( get_the_ID(), 'lsx_faq_url', true );
...
if ( ! empty( $lsx_faq->options['display'] ) && ! empty( $lsx_faq->options['display']['faq_modal_enable'] ) ) {
	if ( ! empty( $lsx_faq->options['display']['faq_modal_cta_label'] ) && ! empty( $lsx_faq->options['display']['faq_modal_form_id'] ) ) {
			$button_label = $lsx_faq->options['display']['faq_modal_cta_label'];
			$button_cf_id = $lsx_faq->options['display']['faq_modal_form_id'];
	}
}
...
if ( $terms && ! is_wp_error( $terms ) ) {
	...
	if ( count( $doc_categories_ ) > 0 ) {
			$connection_faq['post_type'] = 'faq';
			$connection_faq['title'] = esc_html__( 'Related Faq', 'lsx-faq' );

		// $connection_faq['posts'] = get_post_meta( get_the_ID(), 'faq_to_faq', false );
			$connection_faq['posts'] = array();
			...
			$faq_ = new \WP_Query( $args );

		if ( $faq_->have_posts() ) {
			while ( $faq_->have_posts() ) {
					$faq_->the_post();
					$connection_faq['posts'][] = get_the_ID();
					wp_reset_postdata();
			}
		}

		if ( ! empty( $connection_faq['posts'] ) ) {
				$post_ids = join( ',', $connection_faq['posts'] );
				$connection_faq['shortcode'] = '[lsx_faq columns="3" include="' . $post_ids . '"]';
				$connections[] = $connection_faq;
		}
	}
}
...
if ( class_exists( 'WooCommerce' ) ) {
		...
		$connection_product['title'] = esc_html__( 'View the Product', 'lsx-faq' ) . ' <small>' . esc_html__( 'Finished with the faq? Let us head over to the product', 'lsx-faq' ) . '</small>';
		$connection_product['posts'] = get_post_meta( get_the_ID(), 'product_to_faq', false );
	...
	// $connection_service['title'] = esc_html__( 'Services', 'lsx-faq' );
	$connection_service['posts'] = get_post_meta( get_the_ID(), 'service_to_faq', false );
...
if ( ! empty( $connection_service['posts'] ) ) {
	...
	// $connection_testimonial['title'] = esc_html__( 'Testimonials', 'lsx-faq' );
	$connection_testimonial['posts'] = get_post_meta( get_the_ID(), 'testimonial_to_faq', false );
...
if ( ! empty( $connection_testimonial['posts'] ) ) {
	...
	// $connection_team['title'] = esc_html__( 'Team', 'lsx-faq' );
	$connection_team['posts'] = get_post_meta( get_the_ID(), 'team_to_faq', false );
...
<?php
$custom_terms = get_terms( 'faq-category' );
...
?>
					...
					<?php if ( ! empty( $connection_service['small_list_html'] ) ) : ?>
						<div class="entry-meta-key"><?php esc_html_e( 'Services:', 'lsx-faq' ); ?></div>
					...
					<?php if ( ! empty( $connection_product['small_list_html'] ) ) : ?>
						<!--
						<div class="entry-meta-key"><?php esc_html_e( 'Products used:', 'lsx-faq' ); ?></div>
					...
					<?php if ( ! empty( $connection_team['small_list_html'] ) ) : ?>
						<div class="entry-meta-key"><?php esc_html_e( 'Team members involved:', 'lsx-faq' ); ?></div>
						<div class="entry-meta-value entry-meta-value-team"><?php echo wp_kses_post( $connection_team['small_list_html'] ); ?></div>
		
				<?php endif; ?>
					
	<?php if ( ! empty( $url ) ) : ?>
						<div class="entry-meta-single"><a href="<?php echo esc_url( $url ); ?>" target="_blank" rel="nofollow" class="btn btn-block secondary-btn"><?php esc_html_e( 'View Product', 'lsx-faq' ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a></div>
					<?php endif; ?>
	
	
					<?php if ( ! empty( $button_label ) ) : ?>
						<div class="entry-meta-single"><a href="#lsx-faq-contact" data-toggle="modal" class="btn btn-block cta-btn"><?php echo esc_html( $button_label ); ?> <i class="fa fa-angle-right" aria-hidden="true"></i></a></div>
					<?php endif; ?>
				</div>
			</div>
		</div>
  ...
  <ul class="faq"> 
	...
	<?php if ( count( $connections ) > 0 ) : ?>
		<?php foreach ( $connections as $i => $connection ) : ?>
			...
			<div class="lsx-faq-section lsx-full-width">
				<div class="row">
					<div class="col-xs-12">
